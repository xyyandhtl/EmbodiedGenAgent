...
class Walk(VHAction):
    ...
    def change_condition_set(self):
        # del_list = []
        # for c in self.agent_env.condition_set:
        #     if "IsNear" in c:
        #         del_list.append(c)
        # for c in del_list:
        #     self.agent_env.condition_set.remove(c)
        #
        # self.agent_env.condition_set.add(f"IsNear(self,{self.args[0]})")
        self.agent_env.condition_set |= (self.info["add"]) #self.agent_env.condition_set.update(self.info["add"])
        self.agent_env.condition_set -= self.info["del_set"] #self.agent_env.condition_set.difference_update(self.info["del_set"])
